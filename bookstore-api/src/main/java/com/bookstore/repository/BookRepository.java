...
public interface BookRepository extends JpaRepository<Book, Long> {
    ...
    @Query("SELECT DISTINCT b FROM Book b LEFT JOIN b.categories c WHERE " +
           ...
           "(:condition IS NULL OR b.bookCondition = :condition) AND " +
           "(:isRare IS NULL OR b.isRare = :isRare) AND " +
           "(:inStock IS NULL OR (:inStock = true AND b.quantity > 0) OR (:inStock = false))")
